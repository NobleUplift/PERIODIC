...
- **`periodic.table` (draft)** is a single-program replacement. It reads one string list per field (`periodic\elnm`, `elsym`, `elwt`, … `elabp`; item n = atomic number n), stored as values without labels, plus `byname`/`bysym` sort orders. The labels live in `table`'s `Disp` lines. `periodic.mkdata` stores test data for Hydrogen to Boron. Sources are `src/table.txt` (hand-written) and `src/mkdata.txt` (generated by `python3 tools/build-mkdata.py N` from the element programs; don't edit by hand). Rebuild with `python3 tools/ti89-pack.py src/<name>.txt periodic.<name>.89p`, and commit the source and `.89p` together. Sources are UTF-8 in the TI character set: `→` store, `−` negation, `≠`, `©` comment.
