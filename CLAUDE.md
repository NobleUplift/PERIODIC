...
- **`periodic.table` (draft)** is a single-program replacement. It reads one string list per field (`periodic\elnm`, `elsym`, `elwt`, … `elabp`; item n = atomic number n), stored as values without labels, plus `byname`/`bysym` sort orders. The labels live in `table`'s `Disp` lines. `periodic.mkdata` stores the data for atomic numbers 1–110. Sources are `src/table.txt` (hand-written) and `src/mkdata.txt`, generated by `python3 tools/build-mkdata.py [N]` from the element programs (text and compiled) plus `src/manual-values.txt`; don't edit it by hand. Unreadable values become `"?"` and are listed in `src/mkdata-notes.txt`; the user supplies them, and they go into `manual-values.txt`. Rebuild with `python3 tools/ti89-pack.py src/<name>.txt periodic.<name>.89p`, and commit the source and `.89p` together. Sources are UTF-8 in the TI character set: `→` store, `−` negation, `≠`, `©` comment.
...
- These are stored **tokenized** (bytecode, flag `00`), so the diff driver shows them as a hex dump: `periodic`, `atomnum`, and the elements `argon`, `manganes`, `rutheniu`, `techneti`. Their strings are intact as `00 <text> 00 2D` (read them in reverse). They became tokenized after being run on the calculator. Reading them requires GCC4TI's `estack.h` tag tables. They are stored in reverse order: `E8` = new line, `E9` = end, `E4 xx` = a command.
